...
namespace SalonServices
{
    public class AwardService : IAwardService
    {
        ...
        {
            var acceptedEntries = submissions.Sum(sub => sub.Entries.Count(ent => ent.Section.SectionType.IsPrint && OrganisationMatches(ent.Section, orgId)));
        ...
        {
            var acceptedEntries = submissions.SelectMany(sub => sub.Entries.Where(ent => OrganisationMatches(ent.Section, orgId)).Select(ent => ent.ImageId)).Distinct().Count();
        ...
        {
            var acceptedEntries = submissions.SelectMany(sub => sub.Entries.Where(ent => OrganisationMatches(ent.Section, orgId)).Select(ent => ent.Section.SalonYear.SalonId)).Distinct().Count();
        ...
        {
            var acceptedEntries = submissions.SelectMany(sub => sub.Entries.Where(ent => OrganisationMatches(ent.Section, orgId)).Select(ent => ent.Section.SalonYear.Salon.CountryId)).Distinct().Count();
